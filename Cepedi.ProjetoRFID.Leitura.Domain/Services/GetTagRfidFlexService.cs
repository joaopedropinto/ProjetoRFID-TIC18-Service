    ...
    public Task<List<TagRfidModel>> GetTagRfidFlexAsync(int[][] antenas, string ipPorta, string filtro, int tempoLeitura, bool lerMemoriaUsuario, int potenciaPadrao)
    {
        ...
        try
        {
            ...
            using (Reader r = Reader.Create("tcp://" + ipPorta)) //usar URI “IP do leitor:Porta 8081”
            {
                ...
                // Set continuous reading parameters
                r.ParamSet("/reader/read/asyncOnTime", 1000); // Set the duration of reads
                r.ParamSet("/reader/read/asyncOffTime", 0); // Set the delay between reads to 0 for continuous reading
                ...
                {
                    Console.WriteLine("Error: " + args.ReaderException.Message);
                };

                // Start reading asynchronously
                r.StartReading();

                // Wait for the specified read time
                Task.Delay(tempoLeitura).Wait();
